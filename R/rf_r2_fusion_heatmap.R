...
input_file <- 'data/ml_output/rf_results_stats.csv'
nofusion_file <- 'data/ml_output/rf_nofusion_stats.csv'
...
mdl_df <- input_file %>%
  ...
  mutate(
    ...
    struct_pred = factor(
      struct_pred,
      levels = rev(c('TLS + UAS-SfM', 'HMLS + UAS-SfM', 'TLS', 'HMLS', 'UAS-SfM', 'No Struct'))
    ),
    spec_pred = factor(
      spec_pred,
...
ggsave(
  filename = 'figures/r2_fusion_voxel_heatmap.png',
  width = 7.5,
  height = 7.5,
  units = 'in',
  dpi = 700
